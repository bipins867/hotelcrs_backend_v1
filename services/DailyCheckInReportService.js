...
class DailyCheckInReportService {
  ...
  static renderTable(rowsHTML) {
    ...
    return `
      <table>
        ...
        <tbody>
          ${rowsHTML || '<tr><td colspan="20" style="text-align:center">No check-ins scheduled for today</td></tr>'}
        </tbody>
      </table>
    `;
  }

  static async buildHTML(checkIns, generatedAt) {
    const rows = checkIns.map((r) => this.renderRow(r)).join('');
    ...
    return TemplateHelper.loadAndProcessTemplate('daily-checkin-report', dataForTemplate);
  }

  static async sendForDate(targetDate = new Date()) {
    const { checkIns, start } = await this.fetchTodayCheckIns(targetDate);

    // Skip sending if there are no check-ins scheduled today
    if (!checkIns || checkIns.length === 0) {
      return { count: 0, skipped: true };
    }

    const html = await this.buildHTML(checkIns, new Date());
    const subject = `Daily Guest Check-In Report – ${this.formatShort(start)} – WorldCRS.com - World Choice Hotels`;
...
module.exports = DailyCheckInReportService;
