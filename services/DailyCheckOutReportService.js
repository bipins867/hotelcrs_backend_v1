...
class DailyCheckOutReportService {
  ...
  static renderTable(rowsHTML) {
    const headers = [
      ...
      'Hotel Name', 'City', 'State', 'Guest Mobile', 'Guest Email', "Payable Amount to Hotel"
    ];
    return `
      <table>
        <thead><tr>${headers.map(h => `<th>${h}</th>`).join('')}</tr></thead>
        <tbody>
          ${rowsHTML || '<tr><td colspan="17" style="text-align:center">No check-outs recorded today</td></tr>'}
        </tbody>
      </table>
    `;
  }
  ...
  static async buildHTML(checkOuts, generatedAt) {
    ...
    return TemplateHelper.loadAndProcessTemplate('daily-checkout-report', dataForTemplate);
  }

  static async sendForDate(targetDate = new Date()) {
    const { checkOuts, start } = await this.fetchTodayCheckOuts(targetDate);

    // Skip sending if there are no check-outs recorded today
    if (!checkOuts || checkOuts.length === 0) {
      return { count: 0, skipped: true };
    }

    const html = await this.buildHTML(checkOuts, new Date());
    const subject = `Daily Guest Check-Out Report – ${this.formatShort(start)} – WorldCRS.com - World Choice Hotels`;
    const to = emailConfig.adminEmail;

    await EmailService.sendEmail({ to, subject, html });

    return { count: checkOuts.length };
  }
}

module.exports = DailyCheckOutReportService;
